...
@section('content')
    ...
    {!! Form::model($candidate, ['method' => 'PUT', 'route' => ['admin.candidates.update', $candidate->id], 'files' => true,]) !!}
                ...
                <div class="col-xs-12 form-group">
                    ...
                    <div class="photo-block">
                        ...
                        <div class="files-list">
                            @foreach($candidate->getMedia('candidate_resume') as $media)
                                <p class="form-group">
                                    <a href="{{ $media->getUrl() }}" target="_blank">{{ $media->name }} ({{ $media->size }} KB)</a>
                                    <a href="#" class="btn btn-xs btn-danger remove-file">Remove</a>
                                    <input type="hidden" name="candidate_resume_id[]" value="{{ $media->id }}">
                                </p>
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="row">
                <div class="col-xs-12 form-group">
                    @if ($candidate->candidate_photo)
                        <a href="{{ asset(env('UPLOAD_PATH').'/'.$candidate->candidate_photo) }}" target="_blank"><img src="{{ asset(env('UPLOAD_PATH').'/thumb/'.$candidate->candidate_photo) }}"></a>
                    @endif
